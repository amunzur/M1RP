...
df_patient = pd.DataFrame([],columns=['Patient ID']) 
...
sample_dict = {}
checked_genes = ['TP53','FOXA1','PTEN','SPOP']
for p in patients:
    ...
    for index,row in df_one_p.iterrows():
        if df_one_p.at[index,'Sample Type'] == 'PB' and rep_s == '':
            rep[p] = df_one_p.at[index,'Sample ID']
            rep_s = rep[p]
        if df_one_p.at[index,'Cohort'] not in pb_avil and rep_s == '':
            rep[p] = df_one_p.at[index,'Sample ID']
            rep_s = rep[p]
...
for p in patients:
    ...
    rep_mut = set(df_one_p[df_one_p['Sample ID'].isin([rep_s])]['Identifier'].dropna())
    for mut in all_mut:
        gene = mut.split("+")[0]
        effect = mut.split("+")[1]
        ...
        if sample_dict[p] > 1:
            ...
            if mut in rep_mut:
                ...
                df_patient.at[p_id,'Rep has ' + gene + ' mut'] = 'No'
#             print(p_id + ":" + rep_s + ":" + str(per))
            df_patient.at[p_id,'% '+ gene +' mutation'] = per
df_patient = df_patient.apply(lambda x: x.fillna(0) if x.dtype.kind in 'biufc' else x.fillna('NA'))
...
gs = gridspec.GridSpec(1,6,width_ratios=[1,1,1,1,1,0.5])
...
colors = ['#F10707','#3b50b2']

l = 8
sns.swarmplot(x = 'key', y = '% TP53 mutation', data = df_patient_tp53, ax = ax1, size = 5, zorder = 0, hue="Rep has TP53 mut",hue_order=["Yes","No"],order=['M1RP','M1B'], palette=colors)
...
sns.swarmplot(x = 'key', y = '% FOXA1 mutation', data = df_patient_foxa1, ax = ax2, size = 5, zorder = 0, hue="Rep has FOXA1 mut",hue_order=["Yes","No"],order=['M1RP','M1B'],palette=colors)
...
sns.swarmplot(x = 'key', y = '% PTEN mutation', data = df_patient_pten, ax = ax3, size = 5, zorder = 0, hue="Rep has PTEN mut",hue_order=["Yes","No"],order=['M1RP','M1B'],palette=colors)
...
sns.swarmplot(x = 'key', y = '% SPOP mutation', data = df_patient_spop, ax = ax4, size = 5, zorder = 0, hue="Rep has SPOP mut",hue_order=["Yes","No"],order=['M1RP','M1B'],palette=colors)
...
sns.swarmplot(x = 'key', y = '% others mutation', data = df_patient_other, ax = ax5, size = 5, zorder = 0, hue="Rep has others mut",hue_order=["Yes","No"],order=['M1RP','M1B'],palette=colors)
...
ax5.set_xlabel('Others', fontsize=14,labelpad=l)
ax5.legend_.remove()
...
fig.savefig('mutation analysis_1.pdf',bbox_extra_artists=(ax6,), bbox_inches='tight')