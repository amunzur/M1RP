...
jsi_muts = pd.read_csv('G:/Andy Murtha/Ghent/M1RP/dev/heterogeniety indexes/jsi_matrix_mutationOnly.tsv', sep = '\t', index_col='Unnamed: 0')
...
for pt in tc['Patient ID'].unique().tolist():
    ...
    # =============================================================================
    #     Get unique mutation sets
    # =============================================================================
    
    pt_unique_muts = pt_muts.copy()
    pt_unique_muts = pt_unique_muts.drop_duplicates(['GENE','POSITION','EFFECT'])
    pt_unique_muts['mut_number'] = np.arange(len(pt_unique_muts))
    pt_unique_muts['addend'] = pt_unique_muts['mut_number'].apply(lambda x: 2**x)
    pt_unique_muts = pt_unique_muts[['GENE','POSITION','EFFECT','mut_number','addend']]
    pt_mut_sets = pt_muts.copy()
    pt_mut_sets = pt_mut_sets.merge(pt_unique_muts, on = ['GENE','POSITION','EFFECT'])
    pt_mut_sets = pt_mut_sets.groupby('Sample ID').sum().drop_duplicates('addend')
    ...
    dn = dendrogram(ln, p = len(pt_mut_sets), truncate_mode = 'level', ax = ax1, color_threshold = 0, above_threshold_color='k')
    ...
    plt.savefig('G:/Andy Murtha/Ghent/M1RP/dev/heterogeniety indexes/oncoprints/%i/%s_%s_mutsOnly.pdf' % (int(tc_cutoff * 100), cohort, pt),bbox_inches='tight')