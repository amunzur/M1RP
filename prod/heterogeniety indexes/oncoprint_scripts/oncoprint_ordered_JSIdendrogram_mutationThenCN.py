...
jsi_muts = pd.read_csv('G:/Andy Murtha/Ghent/M1RP/prod/heterogeniety indexes/jsi_matrix_mutThenCN.tsv', sep = '\t', index_col='Unnamed: 0')
...
for pt in tc['Patient ID'].unique().tolist():
    ...
    # =============================================================================
    print(pt)
    if pt in ['ID28','ID22','ID29','ID31']: continue;
    ...
    plt.savefig('G:/Andy Murtha/Ghent/M1RP/prod/heterogeniety indexes/oncoprints/%i/%s_%s_mutsThenCN.pdf' % (int(tc_cutoff * 100), cohort, pt),bbox_inches='tight')