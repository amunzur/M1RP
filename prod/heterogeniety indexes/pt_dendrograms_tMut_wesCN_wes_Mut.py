...
wes_cn = pd.read_csv('C:/Users/amurtha/Dropbox/Ghent M1 2019/M1RP Copy Number Analysis/Whole exome (data files, threshold 2.0)/M1RP_wesCNA_pearson_corr.tsv', sep = '\t', index_col = 'Unnamed: 0')
wes_mut = pd.read_csv('G:/Andy Murtha/Ghent/M1RP/prod/heterogeniety indexes/jsi_matrix_WES.tsv', sep = '\t', index_col = 'Unnamed: 0')
...
for pt in ['ID21']:
# for pt in ['ID1']:
    samples = wes_mut[wes_mut.index.str.contains(pt+'_')].index.tolist()
    pt_targeted = targeted[targeted.index.isin(samples)][samples]
    pt_wes_cn = wes_cn[wes_cn.index.isin(samples)][samples]
    pt_wes_mut = wes_mut[wes_mut.index.isin(samples)][samples]
    ...
    [t.set_color('red') for t in ax1.xaxis.get_ticklabels() if t.get_text() in pd.Series(samples).str.split('_').str[2:].str.join('_').tolist()]
