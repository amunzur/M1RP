...
bet = pd.read_csv('C:/Users/amurtha/Dropbox/Ghent M1 2019/Mar2021_datafreeze/mutations/betastasis/M1RP_betastasis_all.tsv', sep = '\t')
...
muts = muts[muts['GENE'].isin(cn['GENE'].unique().tolist()+['FOXA1'])]
...
for pt in ['ID33']:
    ...
    # =============================================================================
    # Remove mutations that are not able to be called in all samples    
    # =============================================================================
    
    tc_pos = pt_tc[pt_tc['Final tNGS_TC'] > 0.1]['Sample ID'].tolist()
    pt_tc = pt_tc.set_index('Sample ID')
    
    pt_bet = bet[index_cols+tc_pos].copy()
    pt_bet = pt_bet.set_index('EFFECT')
    pt_bet = pt_bet[pt_bet.apply(lambda r: r.str.contains('*', regex = False).any(), axis=1)]
    pt_bet = pt_bet.reset_index()
    
    for col in tc_pos:
        pt_bet[col] = pt_bet[col].str.split('(').str[1].str.split(')').str[0].astype(float)
        pt_bet[col] = pt_bet[col] * pt_tc.at[col, 'Final tNGS_TC'] / 2
    ...
    pt_muts = pt_muts.reset_index()
    pt_tc = pt_tc.reset_index()
