# -*- coding: utf-8 -*-
"""
Created on Sun Jun 13 16:11:56 2021
...
sim.loc[sim['similarity'] < 0, 'similarity'] = 0
...
ax.scatter(diff['x'], diff['similarity'], lw = 0, alpha = 0.1, s = 5, c = 'k', zorder = 100)

ax.set_xticklabels(['Same\npatient','Different\npatient'])
ax.set_ylabel('WES CNV Corrlation', fontsize = 6)
...
# =============================================================================
# Get similarity between metastatic and primary
# =============================================================================
...
ax.boxplot([pp['similarity'],mp['similarity'],mm['similarity'],cp['similarity'],mc['similarity']], showfliers = False, zorder = 0)
ax.scatter(pp['x'], pp['similarity'], lw = 0, alpha = 0.5, s = 8, c = 'k', zorder = 100)
ax.scatter(mp['x'], mp['similarity'], lw = 0, alpha = 0.5, s = 8, c = 'k', zorder = 100)
ax.scatter(mm['x'], mm['similarity'], lw = 0, alpha = 0.5, s = 8, c = 'k', zorder = 100)
ax.scatter(cp['x'], cp['similarity'], lw = 0, alpha = 0.5, s = 8, c = 'k', zorder = 100)
ax.scatter(mc['x'], mc['similarity'], lw = 0, alpha = 0.5, s = 8, c = 'k', zorder = 100)

labels = ['Primary\nPrimary\nn=%i' % len(pp),'Primary\nmetastatic\nn=%i' % len(mp),'Metastatic\nmetastatic\nn=%i' % len(mm),'Primary\ncfDNA\nn=%i' % len(cp),'Metastatic\ncfDNA\nn=%i' % len(mc)]

ax.set_xticklabels(labels)
ax.set_ylabel('WES CNV correlation', fontsize = 6)
