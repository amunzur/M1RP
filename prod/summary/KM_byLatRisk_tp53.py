...
lat_0_hit = clin[(clin['latitude_raw'] < 2)&(clin['TP53'] == 2)]
lat_0_intact = clin[(clin['latitude_raw'] < 2)&(clin['TP53'] != 2)]

lat_23_hit = clin[(clin['latitude_raw'] >= 2)&(clin['TP53'] == 2)]
lat_23_intact = clin[(clin['latitude_raw'] >= 2)&(clin['TP53'] != 2)]
...
color = 'blue'
defective_patient_number = str(len(lat_0_hit))
defective_label = str(str('Low risk hit ')+r"(n="+defective_patient_number+")")
T = lat_0_hit['Time_to_CRPC'].round(3)
C = lat_0_hit['crpc_status'].astype(np.int32)
...
defective_label = str(str('Low risk intact ')+r"(n="+defective_patient_number+")")
...
defective_label = str(str('High risk hit ')+r"(n="+defective_patient_number+")")
...
color = 'red'
defective_patient_number = str(len(lat_23_intact))
defective_label = str(str('High risk intact ')+r"(n="+defective_patient_number+")")
T = lat_23_intact['Time_to_CRPC'].round(3)
C = lat_23_intact['crpc_status'].astype(np.int32)
kmf4.fit(T, event_observed = C, label = defective_label)
kmf4.plot(ax=ax,show_censors = True, ci_show = False, color = color, lw = 1)
# lat_23_median=kmf3.median
...
df_test['score'] = 0 # low risk TP53 intact
df_test.loc[(df_test['latitude_raw'] < 2)&(df_test['TP53'] == 2), 'score'] = 1 # Low risk TP53 hit
df_test.loc[(df_test['latitude_raw'] >= 2)&(df_test['TP53'] != 2), 'score'] = 2 # High risk TP53 intact
df_test.loc[(df_test['latitude_raw'] >= 2)&(df_test['TP53'] == 2), 'score'] = 3 # Low risk TP53 hit
...
# plt.savefig('C:/Users/amurtha/Dropbox/Ghent M1 2019/Figures/summary/KM_byLatRisk_TP53.pdf')
# plt.savefig('C:/Users/amurtha/Dropbox/Ghent M1 2019/Figures/summary/KM_byLatRisk_TP53.png')